...
7. **Reference the real implementation.** When documenting behavior (retry schedules, timeouts, headers), check the backend codebase at `../backend` to ensure accuracy (see the `check-backend` skill).
