...
<Update label="July 24, 2026" description="additions">
  ## Icons and permission introspection

  Instruments now carry icon URLs, and you can introspect your effective
  permissions for a realm without hardcoding expected access.

  **Added**
  - `icon_url` field on instrument responses: a public CDN URL to the instrument's icon.
  - [List effective permissions](/api-reference/identity/list-effective-permissions) to retrieve your `resource.action` permissions and their scope in the current realm, including conditional permissions.
  - Error code `duplicate_tenant` when attempting to create a tenant with a name that already exists.
</Update>
