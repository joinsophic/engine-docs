...
<Update label="July 31, 2026" description="additions">
  ## Deeper ETF insight and scoped fee variants

  ETF responses now include fund-level detail for currency and positioning context, and fees support instrument-scoped variants for more granular cost modeling.

  **Added**
  - `display_name`, `headline`, `currency_sentence`, `fund_base_currency`, and `rhp_years` on ETF responses for richer fund context on the [Retrieve an instrument](/api-reference/catalog/retrieve-an-instrument) and [List instruments](/api-reference/catalog/list-instruments) endpoints.
  - `variants` array on Fee responses to scope fees to specific instruments and currencies. Each variant captures how a fee applies within a narrower context.
</Update>
