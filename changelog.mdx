...
<Update label="July 8, 2026" description="additions">
  ...
  **Added**
  - ETF limit orders on [Get an order quote](/api-reference/trading/get-an-order-quote) and [Place an order](/api-reference/trading/place-an-order). Send `limit_price` with `order_type=limit`; quote responses include `max_cost` for buys or `min_proceeds` for sells.
  - Quantity-based ETF buys on [Get an order quote](/api-reference/trading/get-an-order-quote). Send `quantity` instead of `cash_amount`; we persist the quote automatically and return its `id` and `expires_at` for order placement.
...
<Update label="July 3, 2026" description="additions">
  ...
  **Added**
  - `sri` and `hedge_currency` on ETF instruments returned from [List instruments](/api-reference/catalog/list-instruments) and [Retrieve an instrument](/api-reference/catalog/retrieve-an-instrument).
