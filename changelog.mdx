...
<Update label="August 3, 2026" description="additions">
  ## Enhanced fund data and fee variant support

  Instrument responses now include richer fund metadata for better UI presentation,
  and we've added support for fee variants that scope fees to specific instruments.

  **Added**
  - Five new ETF fields on [Retrieve an instrument](/api-reference/catalog/retrieve-an-instrument): `display_name` (short card headline), `headline` (one-line summary), `currency_sentence` (plain-language currency risk description), `rhp_years` (recommended holding period in years from PRIIPs KID), and `fund_base_currency` (currency of the fund's net assets).
  - [Retrieve a fee](/api-reference/fees/retrieve-a-fee) to fetch a fee by ID with its variants.
  - `FeeVariant` schema to represent scoped variations of a fee (e.g. by instrument or instrument type).
  - `variants` array on the `Fee` schema to list all scoped variants of a fee.
</Update>
