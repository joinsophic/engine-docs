...
You'll need sandbox credentials from us. Sandbox execution is enabled on our side for that environment.
...
For a market buy, we derive the order quantity from `cash_amount` using a default price of **100** when you do not set `price` on the order. With `cash_amount: "5002"`, that implies an order quantity of **50.02**. The **trade** leg executes **40%** of that:

| Field | Value |
|---|---|
| `quantity` | `20.008` |
| `remaining_quantity` | `30.012` |
| `total_quantity_executed` | `20.008` |
| `notional` | `2000.80` |
...
Expect `trade.executed` followed by `order.filled`. The trade payload should reflect the 40% partial fill values in the table above (`quantity` `20.008`, `notional` `2000.80`, and so on).
