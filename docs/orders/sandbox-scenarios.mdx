...
| Partial fill | Buy, `cash_amount: "5002"` | `filled` | `order.placed` → `order.executing` → `trade.executed` → `order.filled` |
...
The partial-fill scenario is the easy one to misread. We **do not** end the order in `cancelled` — we record a trade first, then mark the order `filled` because some quantity executed.

When you use `cash_amount: "5002"`, the synthetic trade leg uses these fixed values (useful for assertions in tests):

| Field | Value |
|---|---|
| `quantity` | `600` |
| `remaining_quantity` | `634.56` |
| `total_quantity_executed` | `600` |
| `notional` | `60000` |
...
Expect `trade.executed` followed by `order.filled`.
