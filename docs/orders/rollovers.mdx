...
You can toggle `automatic_rollover` on or off for a position via the [Update Position](/api-reference/positions/update-position) endpoint, but there's a cutoff: changes are locked **4 days before maturity**. After that point, the rollover will proceed as previously configured.
