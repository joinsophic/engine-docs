...
- The **instrument ID** for the security being traded; you can look these up via the [instruments endpoints](/api-reference/catalog/list-instruments)
- Sufficient **cash balance** (for buy orders) or **position units** (for sell orders) in the account

<Tip>
You can check available cash and positions using the [cash balances](/api-reference/holdings/list-cash-balances) and [positions](/api-reference/holdings/list-positions) endpoints before placing an order.
...
<Note>
Order creation is synchronous; we validate the request and return the order immediately. Everything after that (execution, settlement) happens asynchronously. Use [webhooks](/docs/webhooks/overview) or poll the [Retrieve order](/api-reference/trading/retrieve-an-order) endpoint to track progress.
</Note>
...
To list all orders for an account, use the [List orders](/api-reference/trading/list-orders) endpoint. To list only orders that haven't reached a terminal state, use the [List pending orders](/api-reference/trading/list-pending-orders) endpoint; this is useful for building dashboards or monitoring active trades.
