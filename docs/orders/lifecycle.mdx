...
We fire an event whenever an order changes status. Subscribe to [webhooks](/docs/webhooks/overview) for real-time updates. In the sandbox environment, see [Sandbox scenarios](/docs/orders/sandbox-scenarios) for which events each test order produces.
