...
| HTTP | `code` | When |
|---|---|---|
| 400 | `invalid_request` | The request body fails validation — unknown `claim_type`, missing required field, unknown field, or a conditional rule isn't met (e.g. `pep_role` is missing while `is_pep` is `true`). |
