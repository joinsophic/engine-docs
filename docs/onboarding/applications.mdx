...
| Claim or condition | Required |
|---|---|
| On `registration` | One `certificate_of_good_standing` |
| On `registration` | One `certificate_of_shareholders` |
| On `registration` | One `certificate_of_directors` |
| On `registration` | One `financial_statements` |
| On `registration` | One `ownership_structure` |
| On `registered_address` | One `proof_of_registered_address` |
| When `registered_address.operating_address_is_same` is `false` | One `proof_of_operating_address` |
| When `business_nature.has_nominee_shareholders` is `true` | One `nominee_agreement` |
...
Persons declared in a legal authority's `relationships` claim carry their own requirements per `relationship_type`. Related persons never carry consents — all consents come from the application's natural authorities.

Two groups, split on whether the related person holds authority to act on the entity:

**With authority to act** — `director`, `authorised_signatory`:

| | Required |
|---|---|
| Claims | `identity`, `nationalities`, `residence`, `pep_status` |
| Evidence — for `identity` | One `identity_document` of type `passport`, **or** one `national_id_front` **and** one `national_id_back` |
| Evidence — for `identity` | One `selfie` (no `document_type`) |
| Evidence — for `residence` | One `proof_of_address` |
| Evidence — authorising the appointment | One `board_resolution`, attributed to this person |

**Without authority to act** — `shareholder`, `beneficial_owner`, `trustee`, `settlor`:
