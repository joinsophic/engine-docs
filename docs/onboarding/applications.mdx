...
A consent records that a person agreed to a specific [legal document version](/api-reference/legal-documents/list-versions-of-a-legal-document), or made an attestation. See [required consents](#required-consents).
...
Upload files via the [Files API](/api-reference/files/create-a-file), then reference the returned IDs.
...
For every other consent, fetch a live document version via [`GET /legal-documents/{legal_document_id}/versions`](/api-reference/legal-documents/list-versions-of-a-legal-document) and pass its ID. The version's document type must equal the `consent_type`; mismatches return `400 invalid_request`.
