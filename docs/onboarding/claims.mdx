...
A **claim** (`clm_...`) is a typed data point on a [person](/docs/onboarding/persons). Each has a `claim_type`, a `status`, and a `data` payload validated against a strict schema. Applications validate against claims.
...
| `verified` | Confirmed by compliance. `verified_at` populated. |
| `superseded` | Replaced by a newer claim of the same type. `superseded_at` populated. |
...
| `verified` | `superseded` | A newer claim of the same type is created on the same person. |

<Note>
No `pending → superseded` transition. Pending claims can only be verified or deleted.
</Note>

### Supersession

To update already-verified info, submit a fresh claim of the same type. Any prior `verified` claim of that type is superseded in the same request:
...
After the call: the new claim is `pending`, the prior `verified` one (if any) becomes `superseded`, and any still-pending claim of the same type is left untouched.

## Creating a claim

`POST /persons/{person_id}/claims` with a single `claim` object.

<Tip>
Wrap the claim in a `"claim"` envelope. A flat body is rejected with `400 invalid_request`:

```json
{
  "detail": "The request parameters are invalid.",
  "code": "invalid_request",
  "params": [{"path": ["body", "claim"], "detail": "Field required", "code": "missing_param"}]
}
```
</Tip>
...
| `status` | `pending`, `verified`, `superseded` | Optional. |
...
Returns `204 No Content`. Deleting a `verified` or `superseded` claim returns `400 invalid_request`.
