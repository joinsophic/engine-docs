...
<Note>
Supply a `tin` or `tin_exemption_reason` for each tax residency. Neither is accepted at the claim layer but may block the application if your tenant requires a TIN.
</Note>
...
Same shape as [`residence`](#residence). `registered_address` is the legal registration address; `operating_address` is where the business trades from.
...
<Note>
Different `relationship_type` values carry different onboarding requirements — e.g. every `director` must satisfy an identity/selfie/passport bundle. See [related-person requirements](/docs/onboarding/applications#related-person-requirements).
</Note>
