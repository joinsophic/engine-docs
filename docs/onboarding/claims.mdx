...
A **claim** (`clm_...`) is a typed data point on a [person](/docs/onboarding/persons). Each has a `claim_type`, a `status`, and the spec fields at the root, validated against a strict schema. Applications validate against claims.
...
| `verified` | Confirmed by compliance. `verified_at` populated. A verified claim whose `superseded_at` is non-null is the historical row — a newer verified claim of the same type has taken over. |
...
pending ──▶ verified
   │           │
   │           └──▶ verified (historical, superseded_at set)
   └──▶ (deleted)
```

| From | To | Trigger |
|---|---|---|
| *(none)* | `pending` | [`POST /persons`](/api-reference/create-a-person) or [`POST /persons/{person_id}/claims`](/api-reference/create-a-claim-for-a-person). |
| `pending` | *(deleted)* | [`DELETE /persons/{person_id}/claims/{claim_id}`](/api-reference/delete-a-pending-claim). Pending only. |
| `pending` | `verified` | Compliance review. Not self-service. |
| `verified` (current) | `verified` (historical) | A newer verified claim of the same type is recorded. The old row's `status` stays `verified`; its `superseded_at` is set. |

<Note>
There's no `superseded` status — supersession is a derived state. Filter for currently authoritative claims by checking `status == "verified"` and `superseded_at == null`.
</Note>

### Supersession

To update already-verified info, submit a fresh claim of the same type. The new claim is recorded as `pending`; the prior verified row (if any) is retired in the same transaction:
...
After the call: the new claim is `pending`, the prior verified row (if any) keeps `status: "verified"` but gets a non-null `superseded_at`, and any still-pending claim of the same type is left untouched.

## Creating a claim

`POST /persons/{person_id}/claims` with the claim spec as the body. The body is a discriminated union keyed on `claim_type` — there's no outer envelope.
...
The response is the full claim, with the spec fields at the root:
...
| `superseded_at` | ISO-8601 timestamp set when a newer verified claim of the same type takes over, or `null` for the current row. |
| `claim_type` | The claim's type — also the discriminator for the response variant. |
| _spec fields_ | The variant-specific fields (e.g. `is_pep`, `pep_role`, …). See [claim type reference](#claim-type-reference). |

<Note>
The OpenAPI schema exposes one response variant per claim type — `IdentityPersonClaim`, `NationalitiesPersonClaim`, …, `RelationshipsPersonClaim` — discriminated on `claim_type`. SDK generators can emit per-variant types instead of a single dynamic shape.
</Note>

## Listing and retrieving claims

List a person's claims, optionally filtered:
...
| `status` | `pending`, `verified` | Optional. To get only the currently authoritative verified claim for each type, filter `status=verified` and discard rows where `superseded_at` is non-null. |

Retrieve a single claim via [`GET /persons/{person_id}/claims/{claim_id}`](/api-reference/retrieve-a-person-claim).
...
Returns `204 No Content`. Deleting any non-pending claim returns `400 invalid_request` with detail `Only pending claims can be deleted; claim '<id>' has status '<status>'` — that includes verified rows whether or not they've been superseded.
...
<Warning>
Claim types are split between natural and legal persons. Creating a claim whose `claim_type` doesn't match the target's `person_type` returns `400 invalid_request` (e.g. `Claim type 'relationships' is not allowed on natural persons.`). See [claim creation errors](/docs/onboarding/errors#person-creation-errors).
</Warning>
