...
Both environments emit the same [webhook events](/docs/webhooks/overview) and expose them via [`GET /events`](/api-reference/events/list-events). Delivery mechanics (headers, signatures, retries, deduplication) are identical.
